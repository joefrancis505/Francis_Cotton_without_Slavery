# Set the working directory to the script's location
setwd(getSrcDirectory(function(dummy) {dummy}))

# Set a CRAN mirror
options(repos = c(CRAN = "https://cloud.r-project.org"))

# Only include packages that are actually used
packages <- c("sf", "dplyr", "readr", "stringr")

# Install packages not yet installed
installed_packages <- packages %in% rownames(installed.packages())
if (any(installed_packages == FALSE)) {
  install.packages(packages[!installed_packages])
}

# Load the packages
invisible(lapply(packages, library, character.only = TRUE))

# Create necessary directories for output
if (!dir.exists("Data")) {
  dir.create("Data", recursive = TRUE)
  cat("Created directory: Data\n")
}

# Process data for a single year
process_year <- function(year) {
  cat("Processing year:", year, "\n")
  
  tryCatch({
    # Find shapefile
    shp_dir <- paste0("Data/Shapefiles/", year)
    shp_files <- list.files(shp_dir, pattern = "\\.shp$", full.names = TRUE)
    
    if (length(shp_files) == 0) {
      stop(paste("No shapefile found for year", year, "in", shp_dir))
    }
    
    # Read only the attribute data from shapefile, not the geometry
    counties <- st_read(shp_files[1], quiet = TRUE) %>%
      st_drop_geometry() %>%
      select(GISJOIN, STATENAM) %>%
      rename(state = STATENAM)
    
    # Read census data from central file
    census_data <- read_csv("Data/census_NHGIS.csv", show_col_types = FALSE) %>%
      filter(year == as.integer(year))
    
    # Process based on year
    processed_data <- NULL
    if (year == "1840") {
      # Sum up all enslaved columns
      enslaved_cols <- paste0("ACZ0", 13:24)
      processed_data <- census_data %>%
        mutate(enslaved_sum = rowSums(select(., all_of(enslaved_cols)), na.rm = TRUE)) %>%
        transmute(
          GISJOIN,
          census_pop = AC0001,
          enslaved = enslaved_sum,
          improved = NA,
          unimproved = NA,
          land = NA,
          cotton = AB4017
        )
    } else if (year == "1850") {
      processed_data <- census_data %>%
        transmute(
          GISJOIN,
          census_pop = ADQ001,
          enslaved = AE6003,
          improved = ADI001,
          unimproved = ADI002,
          land = improved + unimproved,
          cotton = ADM007 * 400
        )
    } else if (year == "1860") {
      processed_data <- census_data %>%
        transmute(
          GISJOIN,
          census_pop = AG3001,
          enslaved = AH3003,
          improved = AGP001,
          unimproved = AGP002,
          land = improved + unimproved,
          cotton = AGY007 * 400
        )
    } else if (year == "1870") {
      processed_data <- census_data %>%
        transmute(
          GISJOIN,
          census_pop = AJR001,
          enslaved = 0,
          improved = AJU001,
          unimproved = AJU002 + AJU003,
          land = improved + unimproved,
          cotton = AJ1010 * 400
        )
    } else if (year == "1880") {
      processed_data <- census_data %>%
        transmute(
          GISJOIN,
          census_pop = AOB001,
          enslaved = 0,
          improved = AOS001,
          unimproved = AOS002,
          land = improved + unimproved,
          cotton = AOH011 * 400
        )
    } else if (year == "1890") {
      processed_data <- census_data %>%
        transmute(
          GISJOIN,
          census_pop = ASW001,
          enslaved = 0,
          improved = AUJ001,
          unimproved = AUJ002,
          land = improved + unimproved,
          cotton = ATB007 * 477
        )
    } else if (year == "1900") {
      processed_data <- census_data %>%
        transmute(
          GISJOIN,
          census_pop = AWS001,
          enslaved = 0,
          improved = AWU001,
          unimproved = AWT001 - AWU001,
          land = improved + unimproved,
          cotton = (AXO024 + AXO026) * 500
        )
    }
    
    if (is.null(processed_data)) {
      stop(paste("Census data is NULL for year", year))
    }
    
    # Join county data with census data
    counties_df <- counties %>%
      left_join(processed_data, by = "GISJOIN")
    
    # Add year column
    counties_df$year <- as.integer(year)
    
    # Deduplicate by keeping only rows with data
    counties_df <- counties_df %>%
      group_by(GISJOIN) %>%
      arrange(desc(!is.na(census_pop)), .by_group = TRUE) %>%
      slice(1) %>%
      ungroup()
    
    # Adjust cotton calculation for 1880
    if (year == "1880") {
      counties_df <- counties_df %>%
        mutate(cotton = ifelse(state %in% c("Texas", "Arkansas", "Missouri"), 
                               cotton * 500/400, 
                               cotton * 475/400))
    }
    
    # Select only the requested variables
    counties_df <- counties_df %>%
      select(GISJOIN, year, state, census_pop, enslaved, improved, unimproved, land, cotton)
    
    return(counties_df)
  }, error = function(e) {
    cat("Error in process_year function for year:", year, "\n")
    cat("Error message:", conditionMessage(e), "\n")
    return(NULL)
  })
}

# --- Main Function ---
main <- function() {
  cat("=== Starting Data Processing ===\n")
  
  # Define years
  years <- c("1840", "1850", "1860", "1870", "1880", "1890", "1900")
  
  # Process all years
  all_counties_list <- list()
  for (year in years) {
    counties_df <- process_year(year)
    if (!is.null(counties_df)) {
      all_counties_list[[year]] <- counties_df
    }
  }
  
  # Check if any years were processed
  if (length(all_counties_list) == 0) {
    stop("Processing failed for all years. No data to combine.")
  }
  
  # Combine all processed years
  all_counties <- bind_rows(all_counties_list)
  
  # Write the result to a CSV file
  write_csv(all_counties, "Data/cotton_database.csv")
  
  # Print a summary of the processed data
  cat("\nProcessing complete. Summary of the data:\n")
  cat("Total number of rows:", nrow(all_counties), "\n")
  cat("Columns:", paste(colnames(all_counties), collapse=", "), "\n")
  cat("Data for each year:\n")
  print(table(all_counties$year))
  cat("\nCombined data has been written to 'Data/cotton_database.csv'\n")
}

# Run main function
main()